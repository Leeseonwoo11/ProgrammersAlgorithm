// English ending.cpp : 이 파일에는 'main' 함수가 포함됩니다. 거기서 프로그램 실행이 시작되고 종료됩니다.
//
...
bool isin(string s, vector<string> a)
{
	bool result = false;
...
vector<int> solution(int n, vector<string> words) {
	...
	while (count != len)
	{
		...
		if (tempstr.size() > 1) {
			if (temp.front() != tempstr[count - 1].back())
			{
				break;
			}
		}
